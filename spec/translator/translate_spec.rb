...
describe TeX2md::Translator, 'translates' do
  ...
  %w(abbr emph leadin unbreakable).each do |macro|
    describe "\\#{macro}" do
      ...
      it "by wrapping its argument in span.#{macro}" do
        subject.translate(reader, writer)
  ...
  %w(dedication quote signature).each do |environment|
    describe "#{environment} environment" do
      let(:input) { "Before \\begin{#{environment}}in the environment\\end{#{environment}} after" }

      it "by wrapping its contents in div.#{environment}" do
        subject.translate(reader, writer)
  ...
  describe '\story' do
    ...
    it 'by wrapping its argument in h1.story' do
      subject.translate(reader, writer)
  ...
  describe '\chapter' do
    ...
    it 'by writing its argument in h2.chapter' do
      subject.translate(reader, writer)
  ...
  describe '\introduction' do
    ...
    it 'by writing its argument in h2.introduction' do
      subject.translate(reader, writer)
  ...
  describe '\note' do
    ...
    it 'by writing its argument in h3.note' do
      subject.translate(reader, writer)
  ...
  describe 'tilde' do
    ...
    it 'by replacing it with a space' do
      subject.translate(reader, writer)
